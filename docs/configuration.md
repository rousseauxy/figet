...
| `FiGet:InstanceName` | empty | What this copy calls itself among the copies sharing a database, on **Admin → System**. Empty: the machine name, which on a cluster is the pod and in a container is the container's id. It names a *place*, not a process, so a copy that restarts takes its own row back rather than adding one — set it to something stable (`hostname:` in Compose, the pod name through the downward API) where the machine name is not. Give every copy its own: two that share a name share a row, and the page then reports fewer running than there are. |
